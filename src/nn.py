...
use_xors_functions = False

input_dim = 13 if add_label_input else 12
model.add(Dense(12, kernel_regularizer=regularizers.l2(first_layer_decay), input_dim=input_dim))
model.add(Activation("tanh"))
model.add(Dense(10, kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation("tanh"))
model.add(Dense(7, kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation("tanh"))
model.add(Dense(5, kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation("tanh"))
model.add(Dense(4, kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation("tanh"))
model.add(Dense(3, kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation("tanh"))
model.add(Dense(2, kernel_regularizer=regularizers.l2(weight_decay)))
model.add(Activation("tanh"))
model.add(Dense(1, kernel_regularizer=regularizers.l2(weight_decay)))
...
if use_xors_functions:
    all_data = np.array(filter(filtering, all_data))
    x_train = np.array(filter(filtering, x_train))
